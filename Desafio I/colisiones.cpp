...
static void alinearFilaPieza(uint8_t* buffer, uint8_t filaPieza, int x, int bytesPorFila) {
    ...
    int      bitOffset = x % 8;   // desplazamiento dentro del byte
    int      byteOffset = x / 8;  // en que byte empieza la pieza
...
bool puedeColocarPieza(const Tablero* t, const PiezaActiva* pa) {
    ...
    if (pa->x < 0 || (pa->x + f.ancho) > t->ancho) return false;
    ...
    for (int i = 0; i < f.alto; i++) {
        ...
        if (filaTablero >= t->alto) { colision = true; break; }
